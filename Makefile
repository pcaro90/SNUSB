...
# Default target.
all: begin gccversion sizebefore build sizeafter move end
...
OUTFILES = $(TARGET).elf $(TARGET).hex $(TARGET).eep $(TARGET).lss $(TARGET).sym $(TARGET).map
move:
	@echo $(MSG_MOVING)
	$(MOVE) $(OUTFILES) $(OUTDIR)
...
clean clean_list program debug gdb-config move
